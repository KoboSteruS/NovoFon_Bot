...
class AsteriskCallHandler:
    ...
    async def _handle_incoming_call(self, channel_id: str, caller_number: str):
        ...
        try:
            ...
            # Пробуем разные форматы по очереди (alaw/ulaw поддерживаются лучше всего)
            # WAV может содержать G.711 внутри, который Python wave не понимает
            for fmt in ["alaw", "ulaw", "wav", "gsm"]:
                try:
    ...
    async def _read_audio_from_recording(self, channel_id: str, recording_name: str, recording_format: str = None):
        ...
        try:
            ...
            if file_ext == ".wav":
                # Читаем WAV файл
                # ВАЖНО: Asterisk может создавать WAV с G.711 (alaw/ulaw) внутри,
                # который Python wave модуль не понимает
                import wave
                import struct
                ...
                try:
                    ...
                    # Если WAV не читается (например, G.711 внутри WAV контейнера),
                    # пробуем читать как raw PCM или переключаемся на alaw/ulaw
                    logger.warning(f"Failed to read WAV file as standard WAV: {wav_error}")
                    logger.warning(f"Trying to read as raw PCM or switching to alaw/ulaw format")
                    # Пропускаем этот формат, попробуем другой
                    return
